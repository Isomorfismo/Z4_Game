...
# =================== LÓGICA DEL ROMPECABEZAS (Z4 + vecindad Chebyshev) ===================
...
def apply_press_matrix(board, P):
    """Devuelve una copia de board tras aplicar todas las presiones P (valores 0..3)."""
    out = deepcopy(board)
    for r in range(N):
        for c in range(N):
            k = P[r][c] % MOD
...
def mat_inv_mod4(M):
    """Inversa de una matriz cuadrada sobre Z4 mediante Gauss-Jordan (pivotes 1 o 3)."""
    n = len(M)
    ...
    for col in range(n):
        ...
        inv = 1 if A[col][col] % MOD == 1 else 3  # 3*3 ≡ 1 (mod 4)
        for j in range(n):
...
# Matriz de acoplamiento horizontal al bajar una fila (Chebyshev, sin wrap):
# T * p_r = contribución sobre la fila r-1
T = [
    [1,1,0,0],
    [1,1,1,0],
    [0,1,1,1],
...
def solve_min_presses(initial):
    """
    Encuentra P (4x4, valores 0..3) que lleva 'initial' a todo ceros, minimizando el
    número total de +1 (suma de P). Estratégia: enumerate 1a fila (4^4) y propagar con T_INV.
    Devuelve (P_opt, costo). Si no hay solución, devuelve (None, None).
    """
    ...
    for first_row in product(range(4), repeat=4):
        ...
        if any(S[N-1][c] % MOD != 0 for c in range(N)):
            feasible = False
        if not feasible:
            continue
...
def next_hint(board):
    """
    Calcula una 'siguiente jugada' óptima desde el estado actual.
    Regresa (r, c) si hay movimiento, o None si ya está resuelto o sin solución.
    """
    P, cost = solve_min_presses(board)
    if P is None or cost == 0:
        return None
    for r in range(N):
        for c in range(N):
            if P[r][c] % MOD != 0:
...
def optimal_move_sequence(board):
    """
    Devuelve una secuencia de clics unitarios (r,c) que lleva a cero
    con número mínimo de +1. Orden canon: fila por fila, izquierda a derecha.
    """
    P, cost = solve_min_presses(board)
    if P is None:
        return None
...
# Colores y mapeo de flechas
def hex_to_rgb(h):
    h = h.lstrip('#')
    return tuple(int(h[i:i+2], 16) for i in (0, 2, 4))

COLORS = {
    0: hex_to_rgb('#fe0000'),  # arriba
    1: hex_to_rgb('#9aff01'),  # derecha
    2: hex_to_rgb('#01ffff'),  # abajo
    3: hex_to_rgb('#6700ff'),  # izquierda
}
...
class Button:
    ...
    def draw(self, surf, mouse_pos):
        ...
        color = BTN_BG_HOVER if hovering else BTN_BG
...
def draw_arrow(surf, rect, value):
    """Dibuja una flecha llena (triángulo) orientada según value 0=↑,1=→,2=↓,3=← con color."""
    x, y, w, h = rect
    ...
    elif value % 4 == 1:  # derecha
        pts = [(x + w - pad, cy), (x + pad, y + pad), (x + pad, y + h - pad)]
    elif value % 4 == 2:  # abajo
...
def main():
    pygame.init()
    pygame.display.set_caption("Lights Out Z4 - Chebyshev (flechas)")
    ...
    def on_solve():
        ...
        if solving or solved:
            return
        seq = optimal_move_sequence(board)
        if not seq:
            # sin solución (muy improbable aquí); no iniciar
            return
        hint_cell = None
    ...
    buttons.append(Button((GRID_MARGIN, base_y, btn_w, btn_h), "Pista", big, on_hint))
    buttons.append(Button((GRID_MARGIN + btn_w + gap, base_y, btn_w, btn_h), "Resolver", big, on_solve))
    buttons.append(Button((GRID_MARGIN + 2*(btn_w + gap), base_y, btn_w, btn_h), "Nuevo", big, on_new))
    ...
    while running:
        ...
        for r in range(GRID_SIZE):
            for c in range(GRID_SIZE):
                x = GRID_MARGIN + c*TILE
                y = GRID_MARGIN + r*TILE
                rect = (x, y, TILE, TILE)
                # Fondo celda
        ...
        if hint_cell is not None:
            r, c = hint_cell
            hx = GRID_MARGIN + c*TILE
            hy = GRID_MARGIN + r*TILE
            pygame.draw.rect(screen, HINT_BORDER, (hx-3, hy-3, TILE+6, TILE+6), width=3, border_radius=8)
            tip = tiny.render(f"Sugerencia: clic en (fila {r+1}, col {c+1})", True, WHITE)
            screen.blit(tip, (GRID_MARGIN, GRID_MARGIN + GRID_SIZE*TILE + 74))
        ...
        screen.blit(label, (GRID_MARGIN, GRID_MARGIN + GRID_SIZE*TILE + 46))

        # Instrucciones breves
        inst = tiny.render("Haz clic en una casilla para sumar +1 (mod 4) a su vecindad 3x3.", True, (200,200,210))
        screen.blit(inst, (GRID_MARGIN, GRID_MARGIN + GRID_SIZE*TILE + 10))
